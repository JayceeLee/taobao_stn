...
import multipath_stn_inference
...
save_dir = 'home/deepinsight/tongzhen/tmp'
...
tf.app.flags.DEFINE_string(
    'eval_dir', save_dir, 'Directory where the results are saved to.')
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    network_fn = multipath_stn_inference.get_network_fn(NUM_CLASSES, NUM_TRANSFORMER,
    ...
    filename_list, label_list = _get_filename_list(dataset_dir, eval_filename)
    num_samples = len(filename_list)
    filename, label = tf.train.slice_input_producer([filename_list, label_list], num_epochs=1)
